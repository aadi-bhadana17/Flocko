...
const RestaurantDashboard = () => {
    ...
    const fetchOrders = async () => {
        try { setOrders(sortOrdersByMostRecent(await getRestaurantOrders(restaurant.restaurantId))); }
        catch (e) { flashError(e); }
    };
                        ...
                        {foods.map(food => (
                            <div key={food.id} className="rd-entity-card rd-food-card">
                                <div className="rd-food-image-wrap">
                                    <img
                                        src={food.images?.[0] || 'food-placeholder.png'    }
                                        alt={food.name}
                        ...
                        {staffMembers.map((staff) => (
                            <div key={staff.userId} className="rd-list-item rd-staff-row">
                                <div className="rd-list-info">
                                    <span className="rd-list-name">{staff.name}</span>

                                    <span className="rd-list-sub">
                                        {staff.email}
                                    </span>
